-- Updating one pick failed when ANOTHER saved pick's line had moved >1.5 pts:
-- the app resubmits every unlocked pick on save, and the staleness gate
-- treated an unchanged resubmission like a brand-new pick at an old number.
-- A saved pick's number is locked in by design (line shopping), so skip the
-- line validation for any pick that exactly matches a row the member already
-- has saved. New or changed picks are still validated against current lines.
...
BEGIN
  ...
  FOR pick IN SELECT * FROM jsonb_array_elements(p_picks) LOOP
    ...
    -- An unchanged resubmission keeps its saved number: a moved line must
    -- never invalidate a pick the member isn't changing.
    SELECT EXISTS (
