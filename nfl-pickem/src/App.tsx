...
function App() {
  ...
  const loadGames = async () => {
    try {
      ...
      const response = await fetch(`${process.env.PUBLIC_URL}/nfl_lines_week8.csv`);
      ...
      <table className="min-w-full divide-y divide-gray-200 text-xs">
        <thead className="bg-gray-50">
          <tr>
            ...
            {weeks.map(week => (
              <th key={week} className="px-1 py-2 text-center text-xs font-medium text-gray-500 uppercase border-l border-gray-200">
                W{week}
              </th>
            ))}
          </tr>
        ...
        <tbody className="bg-white divide-y divide-gray-200">
            ...
            return (
              <tr key={user.id} className="hover:bg-gray-50">
                ...
                {weeks.map(week => {
                  ...
                  return (
                    <td key={week} className={`px-1 py-2 text-center text-xs border-l border-gray-200 ${isCurrentWeek ? 'bg-blue-50' : ''}`}>
                      {weekPick ? (
          ...
          <tr className="bg-gray-100 border-t-2 border-gray-300 font-semibold">
            ...
            {weeks.map(week => {
              ...
              return (
                <td key={week} className={`px-1 py-2 text-center text-xs font-bold border-l border-gray-200 ${isCurrentWeek ? 'bg-blue-100' : 'bg-gray-100'}`}>
  ...
  const loadResultsData = async () => {
    try {
      ...
      for (let week = 1; week <= maxWeekToLoad; week++) {
        try {
          const weekResponse = await fetch(`${process.env.PUBLIC_URL}/nfl_results_week${week}.csv`);
