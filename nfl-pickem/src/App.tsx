...
function App() {
  ...
  const loadGames = async () => {
    try {
      ...
      const response = await fetch(`${process.env.PUBLIC_URL}/nfl_lines_week3.csv`);
...
function InsightsBeta({ picks, users, games, teamAbbreviations, currentWeek }: InsightsBetaProps) {
  ...
  // Helper function to get all graded team picks (filtered by selected week)
  const getAllGradedPicks = () => {
    const filteredPicks = selectedWeek === 'all' 
      ? picks 
      : picks.filter(userPick => userPick.week === selectedWeek);
...
function NFLTrends({ games, teamAbbreviations, currentWeek }: NFLTrendsProps) {
  ...
  const [visualSelections, setVisualSelections] = useState<{
    homeAway?: 'home' | 'away';
    favoriteUnderdog?: 'favorites' | 'underdogs';
    spreadRange?: string;
  }>({});
  ...
  const loadResultsData = async () => {
    try {
      ...
      // Load Week 1 results
      try {
        const week1Response = await fetch(`${process.env.PUBLIC_URL}/nfl_results_week1.csv`);
        const week1Text = await week1Response.text();
        Papa.parse(week1Text, {
          header: true,
          complete: (results) => {
            const week1Data = results.data.map((row: any) => ({ ...row, week: 1 }));
            allResults.push(...week1Data);
          }
        });
      } catch (e) {
        console.log('Week 1 results not available');
      }

      // Load Week 2 results
      try {
        const week2Response = await fetch(`${process.env.PUBLIC_URL}/nfl_results_week2.csv`);
        const week2Text = await week2Response.text();
        Papa.parse(week2Text, {
          header: true,
          complete: (results) => {
  ...
  // Base filtered results (only by week)
  const getBaseFilteredResults = () => {
    return selectedWeek === 'all' 
      ? resultsData 
      : resultsData.filter(game => game.week === selectedWeek);
  };

  // Get filtered results for different sections based on other section selections
  const getFilteredResultsForSection = (excludeSection?: string) => {
    let filtered = getBaseFilteredResults();
    
    // Apply cross-section filters, but exclude the section we're calculating for
    if (visualSelections.homeAway && excludeSection !== 'homeAway') {
      if (visualSelections.homeAway === 'home') {
        filtered = filtered.filter(game => game.home_ats_result === 'W');
      } else {
        filtered = filtered.filter(game => game.away_ats_result === 'W');
      }
    }

    if (visualSelections.favoriteUnderdog && excludeSection !== 'favoriteUnderdog') {
      filtered = filtered.filter(game => {
        const homeSpread = parseFloat(game.home_spread) || 0;
        const awaySpread = parseFloat(game.away_spread) || 0;
        
        if (visualSelections.favoriteUnderdog === 'favorites') {
          if (homeSpread < 0) {
            return game.home_ats_result === 'W';
          } else if (awaySpread < 0) {
            return game.away_ats_result === 'W';
          }
        } else {
          if (homeSpread < 0) {
            return game.away_ats_result === 'W';
    ...
    if (visualSelections.spreadRange && excludeSection !== 'spreadRange') {
      filtered = filtered.filter(game => {
        const homeSpread = Math.abs(parseFloat(game.home_spread) || 0);
        const awaySpread = Math.abs(parseFloat(game.away_spread) || 0);
        const spread = Math.max(homeSpread, awaySpread);
        
        switch (visualSelections.spreadRange) {
          case 'Field Goal (1-3)':
            return spread >= 1 && spread <= 3;
          case 'Small (3.5-6.5)':
            return spread >= 3.5 && spread <= 6.5;
          case 'Medium (7-10)':
            return spread >= 7 && spread <= 10;
          case 'Large (10.5+)':
            return spread >= 10.5;
          default:
            return true;
  ...
  // Home vs Away ATS Analysis (uses its own data, not affected by its own selection)
  const getHomeAwayATS = () => {
    const results = getFilteredResultsForSection('homeAway');
    let homeWins = 0, awayWins = 0;
    const totalGames = results.filter(game => game.away && game.home).length;
    
    results.forEach(game => {
      if (game.home_ats_result === 'W') homeWins++;
      else if (game.away_ats_result === 'W') awayWins++;
    });
  ...
  // Favorite vs Underdog ATS Analysis (uses its own data, not affected by its own selection)
  const getFavoriteUnderdogATS = () => {
    const results = getFilteredResultsForSection('favoriteUnderdog');
    ...
    results.forEach(game => {
      ...
      if (homeSpread < 0) {
        // Home team favored
        if (game.home_ats_result === 'W') favoriteWins++;
        else if (game.away_ats_result === 'W') underdogWins++;
      } else if (awaySpread < 0) {
        // Away team favored
        if (game.away_ats_result === 'W') favoriteWins++;
        else if (game.home_ats_result === 'W') underdogWins++;
      }
    });
  ...
  // Spread Range Analysis (uses its own data, not affected by its own selection)
  const getSpreadRangeAnalysis = () => {
    const results = getFilteredResultsForSection('spreadRange');
    const ranges: { [key: string]: { picked: number; correct: number } } = {
      'Field Goal (1-3)': { picked: 0, correct: 0 },
      'Small (3.5-6.5)': { picked: 0, correct: 0 },
      'Medium (7-10)': { picked: 0, correct: 0 },
      'Large (10.5+)': { picked: 0, correct: 0 }
    };
    ...
    results.forEach(game => {
      ...
      let category = '';
      if (spread >= 1 && spread <= 3) category = 'Field Goal (1-3)';
      else if (spread >= 3.5 && spread <= 6.5) category = 'Small (3.5-6.5)';
      else if (spread >= 7 && spread <= 10) category = 'Medium (7-10)';
      else if (spread >= 10.5) category = 'Large (10.5+)';
      
      if (category && ranges[category]) {
        ranges[category].picked += 2; // Count both teams
        if (game.home_ats_result === 'W') ranges[category].correct++;
        if (game.away_ats_result === 'W') ranges[category].correct++;
      }
    });
  ...
  // Chart component with click filtering
  const AnalyticsChart = ({ 
    ...
    return (
      <div 
        className={`flex items-center py-2 ${onClick ? 'cursor-pointer hover:bg-gray-50' : ''} ${isActive ? 'bg-blue-50' : ''} ${isGrayedOut ? 'opacity-30' : ''}`}
        onClick={onClick}
      >
        <div className={`font-semibold text-base w-32 text-left ${isGrayedOut ? 'text-gray-400' : 'text-gray-900'}`}>{label}</div>
        <div className={`text-sm w-16 text-center ${isGrayedOut ? 'text-gray-400' : 'text-gray-600'}`}>{data.correct}</div>
        <div className={`text-sm w-16 text-center ${isGrayedOut ? 'text-gray-400' : 'text-gray-600'}`}>{data.picked}</div>
        <div className={`text-sm font-semibold w-20 text-center ${isGrayedOut ? 'text-gray-400' : 'text-gray-900'}`}>{data.percentage}%</div>
        
        <div className="flex-1 ml-6 flex items-center">
          <div 
            className={`h-6 rounded relative ${isGrayedOut ? 'bg-gray-200' : 'bg-gray-300'}`}
            style={{ width: `${barWidth}%`, minWidth: '80px' }}
          >
            <div 
              className={`h-full rounded transition-all duration-500 ${
                isGrayedOut ? 'bg-gray-300' : 
  ...
  const toggleHomeAwayFilter = (type: 'home' | 'away') => {
    setVisualSelections(prev => ({
      ...prev,
      homeAway: prev.homeAway === type ? undefined : type
    }));
  };

  const toggleFavoriteUnderdogFilter = (type: 'favorites' | 'underdogs') => {
    setVisualSelections(prev => ({
      ...prev,
      favoriteUnderdog: prev.favoriteUnderdog === type ? undefined : type
    }));
  };

  const toggleSpreadRangeFilter = (range: string) => {
    setVisualSelections(prev => ({
      ...prev,
      spreadRange: prev.spreadRange === range ? undefined : range
    }));
  };
  ...
  const getTrendingData = () => {
    ...
    getAvailableWeeks().forEach(week => {
      ...
      weekData.forEach(game => {
        if (game.home_ats_result === 'W') homeWins++;
        if (game.away_ats_result === 'W') awayWins++;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="p-4 bg-gray-50 rounded-lg space-y-4">
        ...
        {/* Active Selections */}
        {Object.keys(visualSelections).length > 0 && (
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className="text-sm font-medium text-gray-700">Active Selections:</span>
              <button
            ...
            <div className="flex flex-wrap gap-2">
              {visualSelections.homeAway && (
                <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded text-xs">
                  {visualSelections.homeAway === 'home' ? 'Home Teams' : 'Away Teams'}
                </span>
              )}
              {visualSelections.favoriteUnderdog && (
                <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded text-xs">
                  {visualSelections.favoriteUnderdog === 'favorites' ? 'Favorites' : 'Underdogs'}
                </span>
              )}
              {visualSelections.spreadRange && (
                <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded text-xs">
                  {visualSelections.spreadRange}
                </span>
              )}
      ...
      <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
        <p className="text-sm text-blue-800">
          💡 <strong>Tip:</strong> Click any bar to filter other sections by that category. Click again to clear the filter.
        </p>
      </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        ...
        <AnalyticsChart 
          ...
          label="Home Teams" 
          onClick={() => toggleHomeAwayFilter('home')}
          isActive={visualSelections.homeAway === 'home'}
          isGrayedOut={visualSelections.homeAway === 'away'}
        />
        <AnalyticsChart 
          data={homeAwayStats.away} 
          label="Away Teams" 
          onClick={() => toggleHomeAwayFilter('away')}
          isActive={visualSelections.homeAway === 'away'}
          isGrayedOut={visualSelections.homeAway === 'home'}
        />
      </div>
      ...
      <div className="bg-white p-6 rounded-lg shadow">
        ...
        <AnalyticsChart 
          ...
          label="Favorites" 
          onClick={() => toggleFavoriteUnderdogFilter('favorites')}
          isActive={visualSelections.favoriteUnderdog === 'favorites'}
          isGrayedOut={visualSelections.favoriteUnderdog === 'underdogs'}
        />
        <AnalyticsChart 
          data={favoriteUnderdogStats.underdogs} 
          label="Underdogs" 
          onClick={() => toggleFavoriteUnderdogFilter('underdogs')}
          isActive={visualSelections.favoriteUnderdog === 'underdogs'}
          isGrayedOut={visualSelections.favoriteUnderdog === 'favorites'}
        />
      </div>

      {/* Spread Range Analysis */}
      <div className="bg-white p-6 rounded-lg shadow">
        <h3 className="text-xl font-semibold text-gray-900 mb-6">📊 Spread Range Analysis</h3>
        
        {/* Table Headers */}
        <div className="flex items-center py-2 border-b border-gray-200 mb-4">
          <div className="font-semibold text-sm text-gray-600 w-32 text-left">Range</div>
          <div className="font-semibold text-sm text-gray-600 w-16 text-center">Covers</div>
          <div className="font-semibold text-sm text-gray-600 w-16 text-center">Total</div>
          <div className="font-semibold text-sm text-gray-600 w-20 text-center">Rate</div>
          <div className="font-semibold text-sm text-gray-600 flex-1 ml-6">Performance Distribution</div>
        </div>

        {spreadRangeStats.map(stat => (
          <AnalyticsChart 
            key={stat.range} 
            data={stat} 
            label={stat.range} 
            onClick={() => toggleSpreadRangeFilter(stat.range)}
            isActive={visualSelections.spreadRange === stat.range}
            isGrayedOut={!!(visualSelections.spreadRange && visualSelections.spreadRange !== stat.range)}
          />
        ))}
      </div>
      ...
      <div className="mt-8 p-4 bg-gray-50 rounded-lg">
        ...
        <div className="text-sm text-gray-700 space-y-1">
          ...
          <div>• <strong>Green/Blue fill</strong> = Portion that covered the spread (blue = active filter)</div>
          <div>• <strong>Click any bar</strong> = Filter other sections by that category</div>
          <div>• <strong>Trend charts</strong> = Week-over-week performance comparison</div>
